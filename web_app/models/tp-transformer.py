...
class SelfAttention(nn.Module):
  ...
  def forward(self, query, key, value, mask=None):
    ...
    self.attention_weights = self.dropout(F.softmax(dot, dim=-1))
    # attention = [batch_size, n_heads, seq_size, seq_size]

    v_bar = torch.einsum('bhjd,bhij->bhid', V, self.attention_weights)
...
class DecoderLayer(nn.Module):
  ...
  def forward(self, trg, src, trg_mask, src_mask):
    ...
    self.attention_weights = self.encAttn.attention_weights
    z = self.dropout2(z)
    trg = trg + z
