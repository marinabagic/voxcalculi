...
class SelfAttention(nn.Module):
  ...
  def forward(self, query, key, value, mask=None):
    ...
    self.attention_weights = self.dropout(F.softmax(energy, dim=-1))
    # attention = [batch_size, n_heads, seq_size, seq_size]

    x = torch.einsum('bhjd,bhij->bhid',V,self.attention_weights)
    ...
    return x, self.attention_weights.detach()
